...
variable "bastion_host_cidr" {
  ...
  type        = string
  default     = "209.234.219.138/32" # Your <IP address>/32
}
...
variable "main_api_server_domain" {
  ...
  type        = string
  default     = "hivix.co" # Your backend domain you created a route53 zone for
}

variable "dev_api_server_domain" {
  description = "Dev API Server Domain"
  type        = string
  default     = "api.dev.hivix.co"
}
...
variable "ec2_iam_role_policy_name" {
  ...
  type        = string
  default     = "chatapp-server-ec2-role-polify" # Add a unique name
}
